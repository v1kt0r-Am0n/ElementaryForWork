...
# Script para instalar Docker y Kubernetes en Linux Mint 21.3 (Virginia)

# Verificar root
[ "$(id -u)" -ne 0 ] && echo "Ejecutar con sudo" && exit 1

# Forzar la identificación como Ubuntu 22.04 (base de Linux Mint 21.3)
BASE_CODENAME="jammy"
REAL_CODENAME=$(lsb_release -cs)
...
echo "Distribución: Linux Mint 21.3 (Virginia)"
echo "Base Ubuntu: 22.04 ($BASE_CODENAME)"
echo "Código reportado: $REAL_CODENAME"
...
# Paso 4: Instalar Docker (usando base Ubuntu Jammy)
...
# Configurar repositorio (usando jammy en lugar del código real)
echo "deb [arch=$ARCH signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/ubuntu $BASE_CODENAME stable" | 
